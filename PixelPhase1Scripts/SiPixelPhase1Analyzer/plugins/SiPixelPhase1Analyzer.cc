...
/////////////////////////////////
		...
		{
			FillBins(NULL, theTrackerGeometry, tt);
		}
/////////////////////////////////					
		firstEvent = false;		
	}
...
void SiPixelPhase1Analyzer::BookBarrelHistograms(TDirectory* currentDir, const string& currentHistoName)
{
	...
	for (unsigned i = 0; i < 4; ++i)
	{
		...
		if (opMode == MODE_ANALYZE)
		{
			th2 = new TH2I((histName + std::to_string(i + 1) + "_DEBUG").c_str(), "position", 3000, -30.0f, 30.0f, 1000, -4.5f * (i + 1), 4.5f * (i + 1));
...
void SiPixelPhase1Analyzer::BookForwardHistograms(TDirectory* currentDir, const string& currentHistoName)
{
	...
	{	
		for (unsigned disk = 1; disk <= 3; ++disk)
		{
			...
			th2p = new TH2Poly((histName + std::to_string( (side == 1 ? -(int(disk)) : (int)disk)  )).c_str(), "PXFMap", -15.0, 15.0, -15.0, 15.0);
			...
			if (opMode == MODE_ANALYZE)
			{
				th2 = new TH2I((histName + std::to_string( (side == 1 ? -(int(disk)) : (int)disk)  ) + "_DEBUG").c_str(), "position", 1000, -15.0f, 15.0f, 1000, -15.0f, 15.0f);
...
void SiPixelPhase1Analyzer::BookBarrelBins(ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt)
{
	...
	for (auto &i: pxb)
	{
		...
		SaveDetectorData(true, id.rawId(), pixelBarrelName.shell(), pixelBarrelName.layerName(), pixelBarrelName.ladderName());
#endif
...
void SiPixelPhase1Analyzer::BookForwardBins(ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt)
{
	...
	for (auto &i: pxf)
	{
		...
		SaveDetectorData(false, id.rawId(), pixelEndcapName.halfCylinder(), pixelEndcapName.diskName(), pixelEndcapName.bladeName());
#endif
		
#ifdef DEBUG_MODE
		float r = sqrt(p.x() * p.x() + p.y() * p.y());
		
		debugFile << "Panel: " << panel << "\tSide: " << side << "\tDisk: " << disk << "\tBlade: " << blade << "\tModule: " << module << "\t(x, y, z, r): (" << p.x() << ", " 
																																							<< p.y() << ", "		
																																							<< p.z() << ", "
																																							<< r << ")" << endl;
#endif		
		...
		mat4 transMat(rot.xx(), rot.xy(), rot.xz(),
	...
	for (auto& i: mapOfComplementaryElements) 
	{
		...
		*/
		...
		for (unsigned j = 0; j < 4; ++j)
		{
			...
			if (j > 0)
			{ 
				vertXPanel[0][j - 1] = vertOut[0];	vertYPanel[0][j - 1] = vertOut[1]; 	// for panel 2
			}
			if (j < 3)
			{
				vertXPanel[1][j] = vertOut[0];		vertYPanel[1][j] = vertOut[1];		// for panel 1
			}
		}
...
void SiPixelPhase1Analyzer::SaveDetectorVertices(const TrackerTopology* tt)
{
	...
	{	
		for (unsigned disk = 1; disk <= 3; ++disk)
		{
			std::ofstream* f = new std::ofstream(("vertices_forward_" + std::to_string( (side == 1 ? -(int(disk)) : (int)disk) )).c_str(), std::ofstream::out);
			...
			*(verticesFiles[0][layer - 1]) << rawId << " " << onlineName << " \"";
			for (unsigned i = 0; i < 4; ++i)
			{
				*(verticesFiles[0][layer - 1]) << vertX[i] << "," << vertY[i];
				if (i == 3) *(verticesFiles[0][layer - 1]) << "\"\n";
				else *(verticesFiles[0][layer - 1]) << " ";
			}
		}
		else
		{
			...
			*(verticesFiles[1][mapIdx]) << rawId << " " << onlineName << " \"";
			for (unsigned i = 0; i < 3; ++i)
			{
				*(verticesFiles[1][mapIdx]) << vertX[i] << "," << vertY[i];
				...
				if (i == 2) *(verticesFiles[1][mapIdx]) << "\"\n";
				else *(verticesFiles[1][mapIdx]) << " ";
			}
		}
	}
...
void SiPixelPhase1Analyzer::FillBins(edm::Handle<reco::TrackCollection> *tracks, ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt)
{
	switch (opMode)
	{
		case MODE_ANALYZE:
			for (auto const & track: *(*tracks))
			{
				...
				{					
					...
					const PixelGeomDetUnit* geomdetunit = dynamic_cast<const PixelGeomDetUnit*> ( theTrackerGeometry->idToDet(id) ); // theTrackerGeometry ?????		
					const PixelTopology& topol = geomdetunit->specificTopology();
					
					LocalPoint localPoint = recHit->localPosition();
					GlobalPoint globalPoint = geomdetunit->surface().toGlobal(localPoint);
					
					if (subdetId == PixelSubdetector::PixelBarrel) FillBarrelBinsAnalyze(theTrackerGeometry, tt, id.rawId(), globalPoint);
					else FillForwardBinsAnalyze(theTrackerGeometry, tt, id.rawId(), globalPoint);
				}
			}
			break;
		case MODE_REMAP:
...
void SiPixelPhase1Analyzer::FillBarrelBinsAnalyze(ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt, unsigned rawId, const GlobalPoint& globalPoint)
{
	for (unsigned nameNum = 0; nameNum < baseHistogramName.size(); ++nameNum)
	{
...
#ifdef DEBUG_MODE	
		th2PolyBarrelDebug[strName][layer - 1]->Fill((globalPoint.y() < 0 ? globalPoint.z() + 0.5f : globalPoint.z()), -globalPoint.x(), (globalPoint.y() < 0 ? -1 : 1));
#endif
	}
}

void SiPixelPhase1Analyzer::FillForwardBinsAnalyze(ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt, unsigned rawId, const GlobalPoint& globalPoint)
{
	for (unsigned nameNum = 0; nameNum < baseHistogramName.size(); ++nameNum)
	{
		string strName = baseHistogramName[nameNum];
...
void SiPixelPhase1Analyzer::FillBarrelBinsRemap(ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt)
{
	...
	for (unsigned nameNum = 0; nameNum < baseHistogramName.size(); ++nameNum)
	{
		if (!isBarrelSource[nameNum]) continue;
...
void SiPixelPhase1Analyzer::FillForwardBinsRemap(ESHandle < TrackerGeometry >& theTrackerGeometry, const TrackerTopology* tt)
{
	rootFileHandle = new TFile(analazedRootFileName[0].c_str());
	...
	for (unsigned nameNum = 0; nameNum < baseHistogramName.size(); ++nameNum)
	{
		...
		if (h_2 && h_1)
		{
			...
			for (auto &i: pxf)
			{
				...
				if (pixelEndcapName.ringName() == 1) h = h_1;
				else h = h_2;
...
void 
SiPixelPhase1Analyzer::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
SiPixelPhase1Analyzer::endJob() //MACRO FOR SAVING HISTOGRAMS TO IMAGE FILES
{
#ifdef DEBUG_MODE
  unsigned width = 1500;
  ...
  // BARREL
  for (auto &histGroup: th2PolyBarrel)
  {
    for (auto& hist: histGroup.second)
    {
      string name = (histGroup.first + "#" + hist->GetName());
      TCanvas c(name.c_str(), name.c_str(), width, height);
      
      hist->SetMinimum(zMin);
      hist->SetMaximum(zMax);
      hist->SetStats(0);
      hist->Draw();
      c.Print((name + "_LIN.png").c_str());
      
      c.Clear();
      c.SetLogz();
      hist->Draw();
      c.Print((name + "_LOG.png").c_str());
    }
  }
  
  // FORWARD
  for (auto &histGroup: pxfTh2PolyForward)
  {
    for (auto& hist: histGroup.second)
    {
      string name = (histGroup.first + "#" + hist->GetName());
...
void
SiPixelPhase1Analyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
